...
<body>
<h2 style="margin:0;">Clínic Web — Rol de turnos ({{ $nombreMes }} {{ $anio }})</h2>
<div style="color:#666; font-size: 8px;">Generado: {{ now()->format('Y-m-d H:i') }}</div>

<table class="calendar">
    <thead>
    <tr>
        <th class="doc-head">Doctor</th>
    ...
    @foreach($doctores as $doc)
        <tr>
            ...
            <td class="doc-cell">
                {{ $doc->genero === 'Femenino' ? 'Dra.' : 'Dr.' }} {{ $doc->nombre }} {{ $doc->apellido }}
                <br>
                <span class="muted">{{ $doc->departamento ?? 'General' }}</span>
            </td>

            @foreach($dias as $dia)

                @php
                    $t = $grid[$doc->id][$dia['dia']] ?? null;
                @endphp
